...
class GovernmentAgent:
    ...
    def get_action(self, state):
        ...
        # toutes les actions avec valeur max
    ...
    def learn_expected_sarsa(self, s, a_idx, r, ns):
        s_dis = self.discretize_state(s)
        ns_dis = self.discretize_state(ns)
        ...
        # 1. Identify the current best (greedy) action for the next state
        best_next_action = np.argmax(self.q_table[ns_dis])
        
        # 2. Calculate probabilities under epsilon-greedy policy
        # Each action has a base probability of epsilon / n
        # The greedy action has an additional (1 - epsilon) probability
        probs = np.ones(n_actions) * (self.epsilon / n_actions)
        probs[best_next_action] += (1.0 - self.epsilon)
        
        # 3. Calculate Expected Q-value of the next state
        # This is the sum of (probability of action * Q-value of action)
        expected_q = np.dot(self.q_table[ns_dis], probs)
        
        # 4. Standard TD Update using the Expected Q instead of Max Q
